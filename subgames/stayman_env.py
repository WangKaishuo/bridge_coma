...
class StaymanSubgameEnv:
    ...
    # DDS baseline 的最高阶数限制 (与 Stayman 子博弈的决策空间对齐)
    # 23-27 点牌力下满贯不现实, MAX_LEVEL=4 防止 agent 被不可达的最优惩罚
    MAX_LEVEL = 4
    ...
    def _compute_imp_diff(self) -> float:
        """
        计算 actual vs DDS optimal (受限 MAX_LEVEL) 的 IMP 差值 (≤ 0).
        """
        ...
        optimal = self._get_optimal_contract_ns(dd_table, max_level=self.MAX_LEVEL)
    ...
    def _compute_eval_imp(self) -> float:
        """评估用 IMP: actual vs DDS optimal (受限 MAX_LEVEL, 与训练一致)."""
