...
def load_resume_agent(path: Path, device: str):
    ...
    if "agent" in resume:
        ...
        completed_rounds = int(resume["completed_rounds"])
    else:
        checkpoint = resume
        match = re.search(r"round0*(\d+)", path.name, flags=re.IGNORECASE)
        if match is None:
            raise ValueError(f"Cannot infer checkpoint round from {path}")
...
def main() -> None:
    ...
    parser.add_argument(
        "--allow-round-mismatch",
        action="store_true",
        help="Allow an explicitly cross-budget secondary comparison",
    )
    ...
    if round_a != round_b and not args.allow_round_mismatch:
    ...
    payload = {
        "round": round_a if round_a == round_b else None,
        "round_a": round_a,
        "round_b": round_b,
        "cross_budget_comparison": bool(round_a != round_b),
        "deals": args.deals,
        "seed": args.seed,
        "decision": decision,
