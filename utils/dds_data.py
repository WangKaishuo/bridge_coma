...
Generate compact DDS training data and load it during training.

Compact storage format:
    - decks: uint8 (N, 52), where deck[card] is the owning player (0-3)
    - tricks: int8 (N, 5, 4), where tricks[strain, player] is the DD result
    
    Each sample uses 72 bytes instead of 832 bytes for float32 one-hot data.

Generate data:
    python -m utils.dds_data --num_samples 1000000 --num_workers 8
    python -m utils.dds_data --num_samples 1000000 --seed_offset 1000000
...
def deck_to_hands(deck: np.ndarray) -> np.ndarray:
    """
    Convert compact ownership arrays to one-hot hands.
    
    Args:
        deck: (52,) or (N, 52) uint8
    Returns:
        hands: (4, 52) or (N, 4, 52) float32
    """
    if deck.ndim == 1:
...
class DDSDataLoader:
    """Memory-mapped loader for one dataset file."""
...
class MultiFileLoader:
    """Loader spanning multiple dataset files."""
...
def create_loader(data_path: str, preload: bool = False):
    """
    Select a loader from the input path.
    - file -> DDSDataLoader
    - directory -> MultiFileLoader
    """
    path = Path(data_path)
...
def cards_to_pbn(cards: np.ndarray) -> str:
    """Convert a shuffled deck to PBN without an intermediate one-hot array."""
...
def process_deal(seed: int):
    """Generate one deal and its double-dummy table."""
    try:
...
def generate_all(num_samples: int, batch_size: int, num_workers: int,
    ...
    """Generate the requested dataset."""
...
def analyze(data_path: str):
    ...
    print(f"\nMean DD tricks (NS):")
    for suit, name in enumerate(["C", "D", "H", "S", "NT"]):
...
def main():
    ...
    try:
        ...
        print("Missing dependency: pip install endplay")
        return
