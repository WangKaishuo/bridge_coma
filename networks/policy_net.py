...
Observation generation uses ``pyspiel.State.observation_tensor()`` directly.
...
def convert_hands_suit_to_rank(hands_sm: np.ndarray) -> np.ndarray:
    ...
    Vectorized via a precomputed permutation index to avoid
...
def get_openspiel_game(dealer: int = 0,
    ...
    The cache key includes dealer and vulnerability state.
    """
...
def hands_to_openspiel_state(hands_rm: np.ndarray, dealer: int = 0,
    ...
    SAYC training data always has dealer=North (dealer=0). The SL model
    ...
    Vulnerability is specified as (ns_vul, ew_vul)
    ...
    The interleaved deal-action sequence is cached using the bytes of
    ...
    # Cache the deal sequence because hands_to_deal is constant for a deal.
    cache_key = hands_to_deal.tobytes()
    deal_actions = _deal_action_cache.get(cache_key)
    if deal_actions is None:
        # OpenSpiel deals cards interleaved, not per-player consecutively.
...
# LRU-style cache for deal action sequences
...
def load_sl_into_mappo_agent(agent, sl_checkpoint_path: str):
    """
    Load an OpenSpiel-observation SL checkpoint into a MAPPOAgent.

    Current SL format: {'model_state': state_dict, 'obs_dim': 571, ...}
