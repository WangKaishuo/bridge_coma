...
The OpenSpiel 571-dim observation already contains:
  ...
  - Bidding history: 35 bids × 12 bits (who bid/doubled/redoubled) = 420 dim
...
Architecture:
  ...
  Length head: hidden → 32 (4 suits × 8 bins, softmax per suit)
"""
...
class BeliefNetwork(nn.Module):
    ...
    def compute_loss(
        ...
        - Length [16:48]: CrossEntropyLoss (4 suits × 8 classes)
        """
        logits = self.forward(obs, target_pos)
