...
class StaymanSubgameEnv:
    ...
    def get_action_space(self, history):
        """Stayman 后续的合理选择"""
    ...
    def compute_reward(self, contract, deal):
        """评估定约质量"""
        # 理想：4-4 配合时找到 4M，否则打 3NT
        optimal = self.get_optimal_contract(deal)
...
class CompetitiveSubgameEnv:
    ...
    def compute_info_leakage(self, bid, responder_hand, overcaller):
        """
        计算对手（争叫人）从应叫中获得的信息
        
        用于验证 β 的效果
        """
        # 应叫前后，争叫人对应叫人手牌的推断准确率变化
        return self.belief_net.compute_info_gain(
            observer=overcaller,
            target='responder',
            before=["1H", "2S"],
            after=["1H", "2S", bid]
...
│  阶段 0: 环境与基础设施 (1-2 天)                                 │
│  ├─ 验证 DDS 可以正确计算分数                                   │
│  ├─ 验证 Belief Network 架构可以学习                            │
│  ├─ 验证 IMP 双桌计算正确                                       │
│  └─ 验收标准：单元测试全部通过                                   │
│                                                                 │
│  阶段 1: Stayman 子博弈 (3-5 天)                                 │
│  ├─ 纯合作场景，无竞叫干扰                                       │
│  ├─ 验证 r_info 的 partner term 有效                            │
│  ├─ 验收标准：Belief Accuracy > 0.8, Info Ratio > 1.2           │
│  └─ 预期：DualInfo vs MAPPO 应有 0.3+ IMP 差距                  │
│                                                                 │
│  阶段 2: 竞叫子博弈 (3-5 天)                                     │
│  ├─ 引入对手干扰（1H - 2S - ?）                                 │
│  ├─ 验证 β 的效果                                               │
│  ├─ 验收标准：β=0.5 时 opponent info gain 低于 β=0              │
│  └─ 预期：β=0 vs β=0.5 应有不同行为模式                         │
│                                                                 │
│  阶段 3: 完整叫牌 (5-7 天)                                       │
│  ├─ 使用子博弈调好的超参数                                       │
│  ├─ BC 预热 + Action Mask + 完整训练                            │
│  └─ 验收标准：vs MAPPO +0.3 IMP, Info Ratio > 1.2               │
│                                                                 │
│  Go/No-Go 决策点：                                               │
│  ├─ 阶段 1 后：如果 Info Ratio < 1.0，需重新审视核心假设         │
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Phase 1: 环境构建 (Week 1-3)                                   │
│  ├─ Week 1: 核心环境（发牌、叫牌、DDS）                          │
│  ├─ Week 2: 双桌 IMP + 子博弈环境                               │
│  ├─ Week 3: MAPPO baseline + BC 预热                           │
│  └─ 交付物: 可运行的训练环境                                     │
│                                                                 │
│  Phase 2: 子博弈验证 (Week 4-5) ← 新增                          │
│  ├─ Week 4: Stayman 子博弈实验                                  │
...
| 周次 | 里程碑 | 验收标准 |
|------|--------|----------|
| Week 3 | 环境完成 | 双桌 IMP 可计算；BC 预热后 Pass Rate < 50% |
| Week 4 | Stayman 验证 | Info Ratio > 1.0（Go/No-Go 决策点） |
| Week 5 | 竞叫验证 | β 效果可观测（调整方向决策点） |
...
├── research/                    # 【科研专用】
│   ├── experiments/
│   │   ├── subgame_validation.py  # 【新增】
│   │   ├── baseline_comparison.py
│   │   ├── mechanism_verification.py
│   │   └── ablation_beta.py
│   ├── analysis/
│   │   ├── info_ratio_analysis.py
│   │   ├── temporal_credit_analysis.py  # 【新增】
│   │   └── visualizations.py
│   └── paper/
│
├── engineering/                 # 【工程专用】
